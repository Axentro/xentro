...
component AdminReferrals {
  ...
  fun onAddress (event : Html.Event) {
    next
      {
        selectedAddress = Dom.getValue(event.target),
        updateError = ""
      }
  }
  ...
  fun postCreateReferral (event : Html.Event) {
    sequence {
      ...
      data =
        senderAddress + selectedAddress + publicKey
