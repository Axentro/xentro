...
component PurchaseTokens {
  ...
  fun fetchRates {
    sequence {
      ...
      if (response.status == 200) {
        ...
        next
          {
            sendError = "Could not fetch rates",
            rates = Maybe.nothing()
          }
      }
    } catch {
      next
        {
          sendError = "Could not validate referral code",
          rates = Maybe.nothing()
        }
    }
  }
        ...
        next
          {
            ...
            referralValidationComplete = true,
            sendError = "Referral code was invalid!"
          }
      }
    } catch {
  ...
  fun postMakeOrder (event : Html.Event) {
    sequence {
      ...
      data =
        senderAddress + referralCode + totalAmount + selectedPaymentType + selectedAmount + selectedAgree + publicKey
      ...
      if (response.status == 200) {
        sequence {
          next
            {
              selectedAmount = "Choose",
              selectedPaymentType = "Choose",
              selectedAgree = "Choose"
            }
  ...
  fun onPaymentType (event : Html.Event) {
    next { selectedPaymentType = Dom.getValue(event.target) }
  }

  fun onAgree (event : Html.Event) {
    next { selectedAgree = Dom.getValue(event.target) }
  }

  fun onAmount (event : Html.Event) {
    next { selectedAmount = Dom.getValue(event.target) }
  }
    ...
    <div class="card border-dark mb-3">
      <div class="card-body">
        ...
        <div
          ...
          <p>"You can purchase tokens using ETH or BTC by filling out the form below. After placing your order by clicking 'Purchase' you will be asked to make a transaction to our wallet address for the specified amount and then enter the transaction id into the form."</p>
          ...
          <p>
            "If you wish to pay using an alternative method please select the Alternative option and contact "
            ...
            " with the order number to proceed."
          </p>

          <p>
            "At any time if you have a query please contact "

            <span class="btn btn-sm btn-info">
              "kingsleyh"
            </span>

            " in the Official Telegram channel mentioned above."
          </p>
            ...
            <div class="col">
              ...
              <div
                ...
                <p>
                  "By selecting "
                  <b>"I agree"</b>
                  " from the select box below, you agree that you are not a citizen of America, China or any other country in which making a purchase is illegal."
                </p>
                ...
                <p>
                  "By selecting "
                  <b>"I agree"</b>
                  " you acknowledge that you are making this purchase at your own risk and that the value of your purchase may go up or down and is not intended as an investment of any kind."
                </p>
  ...
  fun totalToPay (currentRates : RatesResponse) : String {
    try {
      if (selectedPaymentType == "ETH") {
        try {
          ((Number.fromString(selectedAmount)
          |> Maybe.withDefault(0)) * (Number.fromString(currentRates.eth)
          |> Maybe.withDefault(0)))
          |> Number.toFixed(8)
        }
      } else if (selectedPaymentType == "BTC") {
        ((Number.fromString(selectedAmount)
        |> Maybe.withDefault(0)) * (Number.fromString(currentRates.btc)
        |> Maybe.withDefault(0)))
        |> Number.toFixed(8)
      } else {
        ((Number.fromString(selectedAmount)
    ...
    <div class="table-responsive">
      <table
        ...
        <tbody>
          ...
          <td>
            <{ rateForPaymentType(currentRates) }>
          </td>

          <td>
            <{ totalToPay(currentRates) }>
          </td>
        </tbody>
